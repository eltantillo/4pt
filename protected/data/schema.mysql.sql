-- MySQL Script generated by MySQL Workbench
-- 02/21/17 14:04:19
-- Model: New Model    Version: 1.0
-- MySQL Workbench Forward Engineering

SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0;
SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0;
SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='TRADITIONAL,ALLOW_INVALID_DATES';

-- -----------------------------------------------------
-- Schema mydb
-- -----------------------------------------------------
-- -----------------------------------------------------
-- Schema 4pt
-- -----------------------------------------------------

-- -----------------------------------------------------
-- Schema 4pt
-- -----------------------------------------------------
CREATE SCHEMA IF NOT EXISTS `4pt` DEFAULT CHARACTER SET utf8 ;
USE `4pt` ;

-- -----------------------------------------------------
-- Table `4pt`.`companies`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`companies` ;

CREATE TABLE IF NOT EXISTS `4pt`.`companies` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `name` TEXT NOT NULL,
  `address` TEXT NOT NULL,
  `phone` TEXT NOT NULL,
  `language` TEXT NOT NULL,
  PRIMARY KEY (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`people`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`people` ;

CREATE TABLE IF NOT EXISTS `4pt`.`people` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `company_id` INT UNSIGNED NOT NULL,
  `first_name` TEXT NOT NULL,
  `middle_name` TEXT NULL DEFAULT NULL,
  `last_name` TEXT NULL DEFAULT NULL,
  `email` TEXT NOT NULL,
  `password` TEXT NOT NULL,
  `phone` TEXT NULL DEFAULT NULL,
  `habilities` TEXT NULL DEFAULT NULL,
  `capabilities` TEXT NULL DEFAULT NULL,
  `roles` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_people_companies_idx` (`company_id` ASC),
  CONSTRAINT `fk_people_companies`
    FOREIGN KEY (`company_id`)
    REFERENCES `4pt`.`companies` (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`projects`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`projects` ;

CREATE TABLE IF NOT EXISTS `4pt`.`projects` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `company_id` INT UNSIGNED NOT NULL,
  `title` TEXT NOT NULL,
  `acronym` TEXT NOT NULL,
  `product_type` INT NOT NULL,
  `level` INT NOT NULL,
  `template` INT UNSIGNED NULL DEFAULT NULL,
  `roles` TEXT NULL DEFAULT NULL,
  `people` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_project_companies1_idx` (`company_id` ASC),
  CONSTRAINT `fk_project_companies1`
    FOREIGN KEY (`company_id`)
    REFERENCES `4pt`.`companies` (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`templates`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`templates` ;

CREATE TABLE IF NOT EXISTS `4pt`.`templates` (
  `id` INT UNSIGNED NOT NULL AUTO_INCREMENT,
  `company_id` INT UNSIGNED NOT NULL,
  `used_times` INT UNSIGNED NULL DEFAULT 0,
  `name` TEXT NOT NULL,
  `scope` TEXT NULL DEFAULT NULL,
  `communication_plan` TEXT NULL DEFAULT NULL,
  `general_purpose` TEXT NULL DEFAULT NULL,
  `specific_objectives` TEXT NULL DEFAULT NULL,
  `justification` TEXT NULL DEFAULT NULL,
  `tool` TEXT NULL DEFAULT NULL,
  `communication_type` TEXT NULL DEFAULT NULL,
  `roles` TEXT NULL DEFAULT NULL,
  `risk` TEXT NULL DEFAULT NULL,
  `impact` TEXT NULL DEFAULT NULL,
  `answer` TEXT NULL DEFAULT NULL,
  `testing` TEXT NULL DEFAULT NULL,
  `people` TEXT NULL DEFAULT NULL,
  `timetable` TEXT NULL DEFAULT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_templates_companies1_idx` (`company_id` ASC),
  CONSTRAINT `fk_templates_companies1`
    FOREIGN KEY (`company_id`)
    REFERENCES `4pt`.`companies` (`id`))
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`processes`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`processes` ;

CREATE TABLE IF NOT EXISTS `4pt`.`processes` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_processes_projects1_idx` (`project_id` ASC),
  CONSTRAINT `fk_processes_projects1`
    FOREIGN KEY (`project_id`)
    REFERENCES `4pt`.`projects` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`work_statement`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`work_statement` ;

CREATE TABLE IF NOT EXISTS `4pt`.`work_statement` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `process_id` INT NOT NULL,
  `product_description` TEXT NULL,
  `scope` TEXT NULL,
  `objectives` TEXT NULL,
  `deliverables` TEXT NULL,
  `sent` TINYINT(1) NULL,
  `project_manager_validated` TINYINT(1) NULL,
  `technical_leader_validated` TINYINT(1) NULL,
  `change_request` TINYINT(1) NULL,
  `change_request_details` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_work_statement_process1_idx` (`process_id` ASC),
  CONSTRAINT `fk_work_statement_process1`
    FOREIGN KEY (`process_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`project_plan`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`project_plan` ;

CREATE TABLE IF NOT EXISTS `4pt`.`project_plan` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `process_id` INT NOT NULL,
  `project_manager_validated` TINYINT(1) NULL,
  `technical_leader_validated` TINYINT(1) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_project_plan_processes1_idx` (`process_id` ASC),
  CONSTRAINT `fk_project_plan_processes1`
    FOREIGN KEY (`process_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`delivery_instructions`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`delivery_instructions` ;

CREATE TABLE IF NOT EXISTS `4pt`.`delivery_instructions` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_plan_id` INT NOT NULL,
  `release_requirements` TEXT NULL,
  `delivery_requirements` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_delivery_instructions_project_plan1_idx` (`project_plan_id` ASC),
  CONSTRAINT `fk_delivery_instructions_project_plan1`
    FOREIGN KEY (`project_plan_id`)
    REFERENCES `4pt`.`project_plan` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`tasks`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`tasks` ;

CREATE TABLE IF NOT EXISTS `4pt`.`tasks` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_plan_id` INT NOT NULL,
  `task` TEXT NULL,
  `duration` DOUBLE NULL,
  `start_date` DATETIME NULL,
  `resources` DOUBLE NULL,
  `people_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_tasks_project_plan1_idx` (`project_plan_id` ASC),
  INDEX `fk_tasks_people1_idx` (`people_id` ASC),
  CONSTRAINT `fk_tasks_project_plan1`
    FOREIGN KEY (`project_plan_id`)
    REFERENCES `4pt`.`project_plan` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_tasks_people1`
    FOREIGN KEY (`people_id`)
    REFERENCES `4pt`.`people` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`risks`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`risks` ;

CREATE TABLE IF NOT EXISTS `4pt`.`risks` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_plan_id` INT NOT NULL,
  `risk` TEXT NULL,
  `cost` DOUBLE NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_risks_project_plan1_idx` (`project_plan_id` ASC),
  CONSTRAINT `fk_risks_project_plan1`
    FOREIGN KEY (`project_plan_id`)
    REFERENCES `4pt`.`project_plan` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`minutes`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`minutes` ;

CREATE TABLE IF NOT EXISTS `4pt`.`minutes` (
  `id` INT NOT NULL ,
  `project_plan_id` INT NOT NULL,
  `purpose` TEXT NULL,
  `assistants` TEXT NULL,
  `date` DATETIME NULL,
  `place` TEXT NULL,
  `previous_minute_id` INT NOT NULL,
  `minutescol` VARCHAR(45) NULL,
  `issues_raised` TEXT NULL,
  `open_issues` TEXT NULL,
  `agreements` TEXT NULL,
  `next_meeting` DATETIME NULL,
  `client_validated` TINYINT(1) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_minutes_project_plan1_idx` (`project_plan_id` ASC),
  INDEX `fk_minutes_minutes1_idx` (`previous_minute_id` ASC),
  CONSTRAINT `fk_minutes_project_plan1`
    FOREIGN KEY (`project_plan_id`)
    REFERENCES `4pt`.`project_plan` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_minutes_minutes1`
    FOREIGN KEY (`previous_minute_id`)
    REFERENCES `4pt`.`minutes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`project_execution`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`project_execution` ;

CREATE TABLE IF NOT EXISTS `4pt`.`project_execution` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_project_execution_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_project_execution_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`progress_report`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`progress_report` ;

CREATE TABLE IF NOT EXISTS `4pt`.`progress_report` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_execution_id` INT NOT NULL,
  `task_status` TEXT NULL,
  `results_status` TEXT NULL,
  `resources_status` TEXT NULL,
  `costs_status` TEXT NULL,
  `calendar_status` TEXT NULL,
  `risks_status` TEXT NULL,
  `deviations_record` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_progress_report_project_execution1_idx` (`project_execution_id` ASC),
  CONSTRAINT `fk_progress_report_project_execution1`
    FOREIGN KEY (`project_execution_id`)
    REFERENCES `4pt`.`project_execution` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`corrective_actions`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`corrective_actions` ;

CREATE TABLE IF NOT EXISTS `4pt`.`corrective_actions` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `project_execution_id` INT NOT NULL,
  `problem` TEXT NULL,
  `solution` TEXT NULL,
  `corrective_actions` TEXT NULL,
  `responsible_id` INT UNSIGNED NOT NULL,
  `open_date` DATETIME NULL,
  `close_date` DATETIME NULL,
  `complete` TINYINT(1) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_corrective_actions_project_execution1_idx` (`project_execution_id` ASC),
  INDEX `fk_corrective_actions_people1_idx` (`responsible_id` ASC),
  CONSTRAINT `fk_corrective_actions_project_execution1`
    FOREIGN KEY (`project_execution_id`)
    REFERENCES `4pt`.`project_execution` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_corrective_actions_people1`
    FOREIGN KEY (`responsible_id`)
    REFERENCES `4pt`.`people` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`project_closure`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`project_closure` ;

CREATE TABLE IF NOT EXISTS `4pt`.`project_closure` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `reception_register` TEXT NULL,
  `date` DATETIME NULL,
  `delivered_elements` TEXT NULL,
  `acceptance_criteria` TINYINT(1) NULL,
  `pending_issues` TEXT NULL,
  `signature` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_project_closure_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_project_closure_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`software_requirements`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`software_requirements` ;

CREATE TABLE IF NOT EXISTS `4pt`.`software_requirements` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `introduction` TEXT NULL,
  `user_interface` TEXT NULL,
  `external_interfaces` TEXT NULL,
  `reliability` TEXT NULL,
  `efficiency` TEXT NULL,
  `maintenance` TEXT NULL,
  `portability` TEXT NULL,
  `interoperability` TEXT NULL,
  `reuse` TEXT NULL,
  `legal` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_software_requirements_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_software_requirements_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`design_restrictions`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`design_restrictions` ;

CREATE TABLE IF NOT EXISTS `4pt`.`design_restrictions` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `software_requirements_id` INT NOT NULL,
  `description` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_design_restrictions_software_requirements1_idx` (`software_requirements_id` ASC),
  CONSTRAINT `fk_design_restrictions_software_requirements1`
    FOREIGN KEY (`software_requirements_id`)
    REFERENCES `4pt`.`software_requirements` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`functionality`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`functionality` ;

CREATE TABLE IF NOT EXISTS `4pt`.`functionality` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `software_requirements_id` INT NOT NULL,
  `description` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_functionality_software_requirements1_idx` (`software_requirements_id` ASC),
  CONSTRAINT `fk_functionality_software_requirements1`
    FOREIGN KEY (`software_requirements_id`)
    REFERENCES `4pt`.`software_requirements` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`user_manual`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`user_manual` ;

CREATE TABLE IF NOT EXISTS `4pt`.`user_manual` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `user_procedure` TEXT NULL,
  `installation_procedure` TEXT NULL,
  `product_description` TEXT NULL,
  `provided_resources` TEXT NULL,
  `required_enviroment` TEXT NULL,
  `problems_report` TEXT NULL,
  `enter_procedure` TEXT NULL,
  `messages` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_user_manual_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_user_manual_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`software_design`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`software_design` ;

CREATE TABLE IF NOT EXISTS `4pt`.`software_design` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `high_lvl_design` TEXT NULL,
  `low_lvl_design` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_software_design_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_software_design_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`traceability_record`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`traceability_record` ;

CREATE TABLE IF NOT EXISTS `4pt`.`traceability_record` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `traceability_recordcol` TEXT NULL,
  `traceability_recordcol1` VARCHAR(45) NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_traceability_record_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_traceability_record_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`operation_manual`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`operation_manual` ;

CREATE TABLE IF NOT EXISTS `4pt`.`operation_manual` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `operation_criteria` TEXT NULL,
  `operative_enviroment` TEXT NULL,
  `security_alerts` TEXT NULL,
  `deployment_sequence` TEXT NULL,
  `faq` TEXT NULL,
  `aditional_sources` TEXT NULL,
  `security_certification` TEXT NULL,
  `guaranty` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_operation_manual_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_operation_manual_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`maintenance_manual`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`maintenance_manual` ;

CREATE TABLE IF NOT EXISTS `4pt`.`maintenance_manual` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `enviroment` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_maintenance_manual_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_maintenance_manual_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`software_component`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`software_component` ;

CREATE TABLE IF NOT EXISTS `4pt`.`software_component` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `related_code` TEXT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_software_component_processes1_idx` (`processes_id` ASC),
  CONSTRAINT `fk_software_component_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


-- -----------------------------------------------------
-- Table `4pt`.`test_report`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `4pt`.`test_report` ;

CREATE TABLE IF NOT EXISTS `4pt`.`test_report` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `processes_id` INT NOT NULL,
  `resume` TEXT NULL,
  `test_case` TEXT NULL,
  `tester_id` INT UNSIGNED NOT NULL,
  `defect_level` TEXT NULL,
  `affected_functions` TEXT NULL,
  `origin_date` DATETIME NULL,
  `resolution_date` DATETIME NULL,
  `solver_id` INT UNSIGNED NOT NULL,
  PRIMARY KEY (`id`),
  INDEX `fk_test_report_processes1_idx` (`processes_id` ASC),
  INDEX `fk_test_report_people1_idx` (`tester_id` ASC),
  INDEX `fk_test_report_people2_idx` (`solver_id` ASC),
  CONSTRAINT `fk_test_report_processes1`
    FOREIGN KEY (`processes_id`)
    REFERENCES `4pt`.`processes` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_test_report_people1`
    FOREIGN KEY (`tester_id`)
    REFERENCES `4pt`.`people` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_test_report_people2`
    FOREIGN KEY (`solver_id`)
    REFERENCES `4pt`.`people` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;


SET SQL_MODE=@OLD_SQL_MODE;
SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS;
SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS;
